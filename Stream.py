...
with col1:
    if st.button("📌 Correlation Matrix"):
        try:
            st.image("Matrix.png", caption="Correlation Matrix", use_container_width=True)
        except:
...
with col2:
    if st.button("📈 Scatter Plot"):
        try:
            st.image("Scatter.png", caption="Scatter Plot (MVAR vs Temperature)", use_container_width=True)
        except:

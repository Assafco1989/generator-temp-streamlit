...
with col1:
    if st.button("📌 Correlation Matrix"):
        try:
            ...
            st.image(matrix_img, caption="Correlation Matrix", use_container_width=True)
        except:
            st.error("❌ matrix.PNG not found.")

with col2:
    if st.button("📈 Scatter Plot"):
        try:
            scatter_img = Image.open("Scatter.PNG")
            st.image(scatter_img, caption="Scatter Plot (MVAR vs Temperature)", use_container_width=True)
        except:
            st.error("❌ Scatter.PNG not found.")
